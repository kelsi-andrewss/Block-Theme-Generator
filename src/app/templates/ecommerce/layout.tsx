...
import { ECOMMERCE_HEADER_JSX_SOURCE, ECOMMERCE_FOOTER_JSX_SOURCE } from "./jsx-sources";
...
function EcommerceLayoutContent({
  ...
  const [headerJsx, setHeaderJsx] = useState(ECOMMERCE_HEADER_JSX_SOURCE);
  const [footerJsx, setFooterJsx] = useState(ECOMMERCE_FOOTER_JSX_SOURCE);
...
export default function EcommerceLayout({
  ...
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: 'var(--color-bg)' }}></div>}>
      <EcommerceLayoutContent>{children}</EcommerceLayoutContent>
    </Suspense>
  );
}
